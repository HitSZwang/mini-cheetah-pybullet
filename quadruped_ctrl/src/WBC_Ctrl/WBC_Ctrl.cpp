...
template<typename T>
void WBC_Ctrl<T>::run(void * input, Quadruped<T> &_quadruped, LegController<T> &_legController, StateEstimatorContainer<T> &_stateEstimator,
                          DesiredStateCommand<T> &_desiredStateCommand, MIT_UserParameters* userParameters){
  ++_iter;
  //std::cout << _iter;
  // Update Model
  _UpdateModel(_stateEstimator.getResult(), _legController.datas);
  
  // Task & Contact Update
  _ContactTaskUpdate(input, _quadruped, _legController, _stateEstimator, _desiredStateCommand, userParameters);

  // WBC Computation
  _ComputeWBC();
  //MIT_UserParameters* _wbic_data->_qddot[i+6]*dt;
  //}
  ...
  // Update Leg Command
  _UpdateLegCMD(_quadruped, _legController, _stateEstimator, _desiredStateCommand, userParameters);
...
template<typename T>
void WBC_Ctrl<T>::_UpdateLegCMD(Quadruped<T> &_quadruped, LegController<T> &_legController, StateEstimatorContainer<T> &_stateEstimator,
                          DesiredStateCommand<T> &_desiredStateCommand, MIT_UserParameters* userParameters){
  LegControllerCommand<T> * cmd = _legController.commands;
  ...
  for (size_t leg(0); leg < cheetah::num_leg; ++leg) {
    ...
    for (size_t jidx(0); jidx < cheetah::num_leg_joint; ++jidx) {
      ...
      //std::cout << " 腿" << leg << "关节" << jidx <<  cmd[leg].tauFeedForward[jidx] << std::endl;
      cmd[leg].kpJoint(jidx, jidx) = _Kp_joint[jidx];
      cmd[leg].kdJoint(jidx, jidx) = _Kd_joint[jidx];
      //if(leg == 0)
      //{
      //    std::cout << cmd[leg].qDes[jidx] << "----";
      //}
  ...
  for(size_t leg(0); leg<4; ++leg){
    ...
    if(_legController.datas[leg].q[2] < 0.3){
      T knee_pos = _legController.datas[leg].q[2]; 
